...
class MastodonCog(commands.Cog):
    ...
    async def get_media_urls(self, toot_id, mastodon_instance):
        try:
            media_list = []
            for media in (await self.get_toot(toot_id, mastodon_instance))[
                "media_attachments"
            ]:
                if media["type"] == "image":
    ...
    async def on_message(self, message):
        if message.author == self.bot.user:
            return
        if (
            message.guild.id not in self.bot.module_access["mastodon"]
            or "http" not in message.content
        ):
            return
    ...
    async def add_node_to_mastodon_node_list(self, ctx, node_url=None):
        ...
        try:
            mastodon = await self.create_mastodon_api_instance(
                node_url, create_type="try"
            )
            mastodon_instance = mastodon.instance()
        except Exception as e:
            return await ctx.send(
                f"An error occured. Maybe the URL is not a Mastodon instance?\n{e}"
            )
    ...
    async def remove_node_from_mastodon_node_list(self, ctx, node_url=None):
        ...
        if not found_url:
            ...
            self.pickle_node_list(self.mastodon_node_list)
            return await ctx.send(
                f'"{found_url}" has been removed from the Mastodon Node List.'
            )
